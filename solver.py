...
class Solver:
    ...
    def PickBranchingVariable(self):
        for v in self.vars:
            if not v.isAssigned():
                logging.info("Branching dl: {} -> {} = {}".format(self.dl+1,v.name,False))
                return v,False
    ...
    def ConflictAnalysis(self,conflict):
        ...
        logging.debug("Resolving: {} with {}".format(assignment.clause,conflict_clause))
        new_clause = assignment.clause.resolve(conflict_clause)
        logging.debug(" -> {}".format(new_clause))
        ...
        while True:
            ...
            if refB.variable.stk_ptr.dl != refA.variable.stk_ptr.dl:
                logging.debug("UIP Because: {}".format(refA))
